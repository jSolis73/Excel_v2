export default function createTable(parent, rowCount, cellCount) {
  ...
  for (let i = 0; i < cellCount + 1; i++) {
    if (i === 0) {
      ...
      trTh += `<div class="table__tr__td-th">${String.fromCharCode('A'.charCodeAt(0) + i - 1)}<div class="table__tr__td-th__resizeX"></div></div>`;
    }
  }
  trTh += '</div>';
  elem.innerHTML += trTh;
  for (let j = 0; j < rowCount; j++) {
    let tr = `<div class="table__tr"><div class="table__tr__tr-th">${j + 1}<div class="table__tr__tr-th__resizeY"></div></div>`;
